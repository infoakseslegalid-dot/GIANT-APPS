...
// we find `{/* Ringkasan keuangan */}`
const searchString = "{/* Ringkasan keuangan */}";
const startIndex = code.indexOf(searchString);
if (startIndex !== -1) {
    // find `</div> // end Reports return`? Wait, I don't have that.
    // Let's just find `      {openUser && (`
    const endIndex = code.indexOf("{openUser && (");
    if (endIndex !== -1) {
