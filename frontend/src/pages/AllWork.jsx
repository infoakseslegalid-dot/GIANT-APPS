...
export default function AllWork() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto" data-testid="all-work-page">
      <div className="mb-6">
        <h1 className="font-heading text-2xl sm:text-3xl font-bold tracking-tight text-[#172B4D]">Semua Pekerjaan</h1>
        <p className="text-sm text-[#44546F] mt-1">Bank data seluruh pekerjaan lintas divisi.</p>
      </div>

      <div className="flex flex-wrap items-center gap-2 mb-4">
        <div className="flex items-center gap-2 bg-white border border-[#DFE1E6] rounded-lg px-3 h-9 flex-1 min-w-[220px]">
      ...
      <div className="bg-white rounded-xl border border-[#DFE1E6] shadow-sm overflow-x-auto">
        <table className="w-full text-sm" data-testid="allwork-table">
          <thead>
            <tr className="border-b border-[#DFE1E6] text-left">
              <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-[#8590A2]">Pekerjaan</th>
              <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-[#8590A2]">Klien</th>
              <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-[#8590A2]">Board / List</th>
              <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-[#8590A2]">PIC</th>
              <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-[#8590A2]">Tenggat</th>
              <th className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-[#8590A2]">Prioritas</th>
          ...
          <tbody>
            {(items || []).map((item) => (
              <tr
                ...
                className="border-b border-[#F1F2F4] hover:bg-[#F8F9FA] cursor-pointer transition-colors"
              >
                <td className="px-4 py-2.5 font-medium text-[#172B4D] max-w-[280px]">
                  <span className="line-clamp-2">{item.title}</span>
                </td>
                <td className="px-4 py-2.5 text-[#44546F]">{item.client_name || "—"}</td>
                <td className="px-4 py-2.5">
                  ...
                  <span className="text-[#44546F] text-xs ml-1.5">{item.list_name}</span>
                </td>
                <td className="px-4 py-2.5">
        ...
        {isLoading && <p className="p-6 text-sm text-[#44546F]">Memuat...</p>}
        {isError && <p className="p-6 text-sm text-[#CA3521]">Gagal memuat data.</p>}
        {!isLoading && (items || []).length === 0 && (
          <p className="p-6 text-sm text-[#8590A2] text-center">Tidak ada pekerjaan yang cocok.</p>
        )}
      </div>
